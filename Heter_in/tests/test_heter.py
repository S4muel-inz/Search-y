...
import pytest
import os
from ..src.heter import search # Ajuste o nome do import para o seu arquivo
...
def test_search_depth_zero(fs_setup):
    ...
    resultado = list(search(str(fs_setup), depth=0))
...
def test_search_depth_one(fs_setup):
    ...
    resultado = list(search(str(fs_setup), depth=1))
    nomes = [item["name"] for item in resultado]
...
def test_search_filter_files(fs_setup):
    ...
    resultado = list(search(str(fs_setup), typesearch="file", depth=1))
...
def test_search_filter_dirs_plural(fs_setup):
    ...
    resultado = list(search(str(fs_setup), typesearch="dirs", depth=1))
...
def test_search_pattern_match(fs_setup):
    ...
    resultado = list(search(str(fs_setup), Pattern={"imagem"}))
    assert len(resultado) == 1
...
def test_search_pattern_case_insensitive(fs_setup):
    ...
    resultado = list(search(str(fs_setup), Pattern={"ARQUIVO"}))
...
def test_search_permission_error(tmp_path):
    ...
    with patch("os.scandir") as mock_scandir:
        ...
        resultado = list(search(str(pasta)))
...
def test_search_os_error_handling(tmp_path, capsys):
    ...
    with patch("os.scandir") as mock_scandir:
        ...
        resultado = list(search(str(tmp_path)))
